...
HistoricalChart <- function(stock = "AXISBANK.NS",
  ...
  mat$Bench = bench.mat$Close[mtch]
  ...
  plt1 = mat %>%
    plot_ly(x = Date, y = Close, name = "Hist. Prices", line = list(width = 2)) %>%
  ...
  rollCor.plot = data.frame(Date = rev(mat$Date),
                            roll1 = addNA(mat$Close, rollCor[[1]]),
                            roll2 = addNA(mat$Close, rollCor[[2]]),
                            roll3 = addNA(mat$Close, rollCor[[3]]),
                            roll4 = addNA(mat$Close, rollCor[[4]]))
  ...
  MA.mat = data.frame(Date = rev(mat$Date),
                      ma1 = VWMA(rev(mat$Close), rev(mat$Volume), n = width[1]),
                      ma2 = VWMA(rev(mat$Close), rev(mat$Volume), n = width[2]),
                      ma3 = VWMA(rev(mat$Close), rev(mat$Volume), n = width[3]),
                      ma4 = VWMA(rev(mat$Close), rev(mat$Volume), n = width[4]),
                      Close = rev(mat$Close),
                      High = rev(mat$High),
                      Low = rev(mat$Low))
  ...
  plt4 = MA.mat %>% 
    ...
    add_trace(x = Date, y = Close, name = "Hist. Prices", line = l, fill = "tonexty", yaxis = "y2") %>%
